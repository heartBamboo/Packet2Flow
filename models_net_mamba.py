...
class NetMamba(nn.Module):
    def __init__(self, img_size=40, stride_size=4, in_chans=1,
        ...
        num_cls_token = 1
        self.pos_embed = nn.Parameter(torch.zeros(1, 8 + num_cls_token, embed_dim))
        # Mamba blocks
    ...
    def randow_mask_mamba(self, x,mask_ratio):
        N, L, Emb = x.size()
    ...
    def forward_encoder(self, x, mask_ratio, if_mask=True,):
        ...
        if if_mask:
            x, mask, ids_restore = self.randow_mask_mamba(x, mask_ratio)
            #x, mask = self.randow_mask_mamba(x, mask_ratio)

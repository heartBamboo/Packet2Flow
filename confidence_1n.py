...
from utils import setup_seed, ModelTrainer, AverageMeter, make_logger, plot_confusion_matrix, visualize, \
    compute_metrics, compute_metrics_1n, plot_convergence, plot_one
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='./new_datasets/ISCX_TOR_1n_confidence',help='Path to the dataset')
    ...
    parser.add_argument("--lr", default=0.0042257332354428737 , type=float, help="initial learning rate")
    ...
    parser.add_argument("--finetuned_model_path", default='./Finetune_result/2025-03-26_18-10-36/finetune_checkpoint_22_batchsize_32_acc_0.9854921102523804_dataname-ISCX_TOR_2017_only_time256_w8.pth', type=str)
    parser.add_argument("--mode", default='just_evaluate', type=str,choices=['to_pretrain','to_finetune','just_evaluate','only_wtconv'])
    parser.add_argument("--bert_dropout", default=0.0001, type=float, help='dropout rate')
    parser.add_argument("--wt_dropout", default=0, type=float, help='dropout rate')
    ...
    parser.add_argument(
        "--wd",
        "--weight-decay",
        default=0.00005463241085672687,#1e-2--5e-4
        type=float,
        metavar="W",
    ...
    if mode=='to_finetune':
        ...
        logger.info('微调完成')
        start_time = time.time()
        loss_m_valid, predict_label, true_label, low_confidence_sample_index, high_confidence_sample_index = ModelTrainer.evaluate(test_dataloader, model, criterion, device, logger, args)
        end_time = time.time()
        print("eval_time", end_time - start_time)
        #visualize(predict_label, true_label, lable_name, log_dir, 'bert')
        test_metric = compute_metrics(predict_label, true_label,low_confidence_sample_index, high_confidence_sample_index, args, device)
        logger.info(
            'Metrics: Accuracy={metrics[accuracy]:>5.4f} Precision={metrics[precision]:>5.4f} '
            'Recall={metrics[recall]:>5.4f} F1 Score={metrics[f1_score]:>5.4f}'
        ...
        print("eval_time", end_time - start_time)
        #visualize(predict_label, true_label, lable_name, log_dir, 'bert')
        test_metric = compute_metrics_1n(predict_label, true_label,low_confidence_sample_index, high_confidence_sample_index, args, device)
        logger.info(
            'Metrics: Accuracy={metrics[accuracy]:>5.4f} Precision={metrics[precision]:>5.4f} '
            'Recall={metrics[recall]:>5.4f} F1 Score={metrics[f1_score]:>5.4f}'
            .format(metrics=test_metric))

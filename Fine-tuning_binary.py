...
from utils import setup_seed, ModelTrainer, AverageMeter, make_logger, plot_confusion_matrix, visualize, \
    compute_metrics, plot_convergence, compute_metrics_binary
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='./new_datasets/ISCX_VPN_2017_time256_w8_binary',help='Path to the dataset')
    ...
    parser.add_argument("--output-dir", default="./Finetune_result_binary/vpn", type=str, help="path to save outputs")
    ...
    parser.add_argument("--mode", default='to_finetune', type=str,choices=['to_pretrain','to_finetune','just_evaluate'])
    parser.add_argument("--bert_dropout", default=0.05, type=float, help='dropout rate')
    parser.add_argument("--wt_dropout", default=0.05, type=float, help='dropout rate')
    ...
    if mode=='to_finetune':
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            loss_m_valid, predict_label,true_label= ModelTrainer.validate(valid_dataloader, model,criterion,device, logger, args)
            ...
            #绘制混淆矩阵
            true_label_tensor = torch.from_numpy(true_label).squeeze()
            all_preds_tensor = torch.from_numpy(predict_label).squeeze()
            # print('true_label_tensor',true_label_tensor)
            # print('all_preds_tensor',all_preds_tensor)
            pred_probs = torch.sigmoid(all_preds_tensor)
            # print('pred_probs', pred_probs.shape)
            # print(pred_probs)
            pred_labels = (pred_probs > 0.5).to(int)
            # exit()
            # exit()
            #_, all_preds_tensor = torch.max(all_preds_tensor, 1)
            cm = confusion_matrix(true_label_tensor, pred_labels)
            plot_confusion_matrix(cm, classes=lable_name, normalize=True,save_path=log_dir)

            metric = compute_metrics_binary(pred_probs,true_label_tensor, device)
